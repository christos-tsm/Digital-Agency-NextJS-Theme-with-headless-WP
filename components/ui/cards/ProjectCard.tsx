    ...
    return (
        <article id={`project-${project.ID}`} className={`group relative flex! flex-col swiper-slide overflow-hidden rounded-2xl ${projectCardClassName}`}>
            ...
            <div className={`inline-flex ${imageContainerClassName} w-full relative`}>
                <div className="absolute top-0 left-0 w-full h-full bg-foreground/70 z-2"></div>
                <Image src={project.featured_image_url} width={650} height={650} alt={project.post_title} className={`w-full ${imageClassName} object-cover`} />
            </div>
